...
| Profile photo     | `src/assets/profile.jpg`              | — (imported directly in `index.astro`, fully optimized to WebP) |
...
> **Note:** Images in `src/assets/` are fully optimized by Astro at build time (WebP conversion, resizing). Images in `public/images/` are served as-is but include `loading="lazy"` and explicit dimensions in the HTML.
...
src/
├── assets/
│   └── profile.jpg         # Profile photo — imported for full WebP optimization
├── components/
│   ├── Nav.astro           # Site navigation + dark mode toggle (responsive)
│   ├── Footer.astro        # Footer with RSS and GitHub links
│   ├── PostCard.astro      # Blog post card with reading time (used in listings)
...
├── layouts/
│   ├── Base.astro          # Root HTML shell (nav, footer, dark mode, SEO, Umami analytics)
│   ├── BlogPost.astro      # Blog post layout (reading time, prev/next nav, share buttons, code copy)
...
│   └── rss.xml.js          # RSS feed
├── utils/
│   └── readingTime.ts      # Reading time utility (200wpm estimate from body text)
└── styles/
    └── global.css          # Tailwind imports + prose styles + dark mode + code copy button
public/
...
Umami (privacy-friendly, no cookies) is embedded in `Base.astro`. The dashboard is at [umami.is](https://umami.is). No configuration needed — all pages are tracked automatically.
